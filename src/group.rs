...
pub trait FiniteGroupLike<A>: GroupLike<A> {
    ...
    /// as well as a map from the generators to their representing permutations.
    /// # Runtime
    /// Approximately O(n^2).
    fn cayley_embedding(self, gens: Vec<A>) -> (PermutationSubgroup, HashMap<A, Permutation>) where Self: Sized, A: Eq + std::hash::Hash + Clone {
        let em = self.all_elements().collect::<Vec<_>>();
        let elems: bimap::BiHashMap<usize,_> = (0usize..).zip(self.all_elements()).collect();
        let size = elems.len();
        let mut map: HashMap<A, Permutation> = HashMap::new();
