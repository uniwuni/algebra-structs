...
use crate::symmetric_group::*;
use crate::misc::NNInf;
use crate::misc::Subobject;
...
/// Group which is able to construct its finitely generated subgroups.
pub trait CanGenerateSubgroups<A>: Sized {
    /// Make sure the implementations of [`GroupLike`] actually line up.
    /// In particular, for `g: Self::Subgroups`, `Self::op(x,y) = g::op(x,y)`
    /// should either hold or the latter call should panic (if `x` or `y` are not in `g`).
    ///
    /// Since determining whether an element is in a finitely generated subgroup can be intensive computationally,
    /// the panic is not to be relied on.
    type Subgroups: GroupLike<A> + Subobject<A, Self>;

    /// Return an object representing the subgroup generated by `gens`.
    fn generate_group(&self, gens: &Vec<A>) -> Self::Subgroups;
}
...
pub trait FiniteGroupLike<A>: GroupLike<A> {
    ...
    /// Returns a subgroup of some symmetric group that is isomorphic to the subgroup generated by `gens`
    /// as well as a map from the group elements to their representing permutations.
    /// # Runtime
    /// Approximately O(n^2).
    fn cayley_embedding(self, gens: Vec<A>) -> (PermutationSubgroup, bimap::BiHashMap<A, Permutation>) where Self: Sized, A: Eq + std::hash::Hash + Clone {
        let em = GroupGenIter::new(Box::new(&self), &gens).collect::<Vec<_>>();
        let elems: bimap::BiHashMap<usize,_> = (0usize..).zip(em.clone()).collect();
        let size = elems.len();
        let mut map: bimap::BiHashMap<A, Permutation> = bimap::BiHashMap::new();
        let mut pgens: Vec<Permutation> = Vec::new();
        for gen in gens {
            let a: Vec<_> = em.clone().into_iter().map(|a| elems.get_by_right(&self.op(&gen, &a)).unwrap()).copied().collect();
            let p = Permutation {vec: a, degree: size};
            pgens.push(p);
        }
        for gen in em.clone() {
            let a: Vec<_> = em.clone().into_iter().map(|a| elems.get_by_right(&self.op(&gen, &a)).unwrap()).copied().collect();
            let p = Permutation {vec: a, degree: size};
            map.insert(gen, p.clone());
        }
        (PermutationSubgroup::from_gens(SymmetricGroup { degree: size }, pgens), map)
    }
}
...
pub struct GroupGenIter<'a, A> {
    elems: HashSet<A>,
    queue: VecDeque<A>,
    grp: Box<&'a dyn GroupLike<A>>,
    gens: &'a Vec<A>
}

impl<'a, A> GroupGenIter<'a, A> {
    pub fn new(grp: Box<&'a dyn GroupLike<A>>, gens: &'a Vec<A>) -> Self {
        GroupGenIter {
            elems: HashSet::new(),
            queue: VecDeque::from([grp.id()]),
            grp,
            gens
        }
    }
...
impl<'a, A> Iterator for GroupGenIter<'a, A> where A: Clone + Eq + std::hash::Hash {
    type Item = A;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            self.queue.extend(self.gens.iter().map(|s| self.grp.op(&x, s)));
            return Some(x);
        }
    ...
    A: Clone + Eq + std::hash::Hash + PartialEq,
{
    ...
    fn all_elements(&self) -> impl Iterator<Item = A> {
        GroupGenIter {
            ...
            grp: Box::new(&self.fin_gen_group.group),
            gens: &self.fin_gen_group.generators
        }
    }
}
...
mod test {
    ...
    #[test]
    fn elems_mod_1000_cayley() {
        const K : i64 = 1021;
        let grp_mod_5: Group<i64> = Group {
            op_fun: Box::new(|x, y| (x * y) % K),
            id_val: 1,
            inv_fun: Box::new(|x| (-x) % K), // TODO
        };
        let finite_grp_mod_5: FiniteGroup<i64> = FiniteGroup {
            fin_gen_group: FinitelyGeneratedGroup {
                group: grp_mod_5,
                generators: vec![], // irrelevant
            },
        };
        let (g,m) = finite_grp_mod_5.cayley_embedding(vec![212]);
        let inv700 = m.get_by_right(&g.inv(m.get_by_left(&212).unwrap())).unwrap();
        assert_eq!((inv700 * 212) % K, 1);
        assert_eq!(g.order_of(m.get_by_left(&1).unwrap()),crate::misc::NNInf::Fin(BigUint::from(0u8)));
    }
    #[test]
    fn elems_sym_3() {
